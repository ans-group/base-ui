...
const SelectBase = ({
  ...
  return (
    <div
      tabIndex={searchable ? null : 0}
      onBlur={searchable ? () => { } : onBlur}
      onClick={!disabled && onClick}
      className={`${classes} ${className || ''}`}
      {...otherProps}
    >
      {valueIsSet ? renderValue : (!searchable && placeholder)}
      {searchable && <div onBlur={onBlur} contentEditable={true} placeholder={placeholder} className={styles.search} ref={searchNode} />}
    </div>
  )
}
...
const Select = forwardRef(({
  ...
  if (ref) ref.current = { value: Array.isArray(selected) ? selected.map(v => v.value) : (selected && selected.value), name }
  ...
  const handleBlur = e => {
    setTimeout(() => {
      if (!mainNode.current.contains(document.activeElement)) {
        setOpen(false)
      }
    }, 1)
    if (onBlur) {
  ...
  const handleSelect = selectedValue => () => {
    if (!selectedValue || (selectedValue && (selectedValue.text === '' || selectedValue.text === ' '))) return
    selectedValue = typeof selectedValue === 'string' ? { value: selectedValue, text: selectedValue } : selectedValue
    if (multiple) {
      const value = Array.isArray(selected) && selected.map(o => o.value).includes(selectedValue.value)
        ? (allowCustom ? '' : selected.filter(item => item.value !== selectedValue.value))
        : selected.concat(selectedValue)
      if (value) {
        selectItem(value.filter(Boolean))
      }
    } else {
  ...
  }, [selected, multiple, name, onChange])
  ...
  const renderOption = (option, i) => {
    ...
    return (
      <div key={option.value} tabIndex={-1} onClick={handleSelect(option)} className={classes}>
        {option.text}
      </div>
    )
  }
  ...
  return (
    <div className={classes} ref={mainNode} style={style}>
      <SelectBase open={open} selected={selected} loading={loading} searchable={searchable} searchNode={searchNode} {...otherProps} onKeyUp={handleKeyUp} onKeyDown={handleKeyDown} handleRemove={handleRemove} onClick={toggleOpen} onBlur={handleBlur} className={className} />
      {open && <div className={styles.picker}>
        {allowCustom && searchQuery && (
          <div onClick={handleSelect(searchQuery)} className={highlightedIndex === -1 ? styles.highlighted : ''} tabIndex={-1}>
